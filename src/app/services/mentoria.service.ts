...
export class MentoriaService {
  ...
  /**
   * Obter Linha do Tempo por token público
   */
  obterLinhaDoTempoPublico(token: string): Observable<ApiResponse<any>> {
    return this.http.get<ApiResponse<any>>(`${this.apiUrl}/publico/${token}/linha-do-tempo`);
  }

  /**
   * Salvar Linha do Tempo (via token público)
   */
  salvarLinhaDoTempo(token: string, dados: {
    birthdate: string;
    events: { [ano: string]: Array<{ id: number; text: string }> };
    pergunta_trajetoria: string;
    pergunta_proximos_5_anos: string;
  }): Observable<ApiResponse<any>> {
    return this.http.post<ApiResponse<any>>(
      `${this.apiUrl}/publico/${token}/linha-do-tempo`,
      dados
    );
  }
